    ...
    $.ajax({
      ...
      success: function (html) {   
          ...
          if(!html.confirm){
            ...
            var long = html.data;
            //alert(html.data+short)
            postinrbmq(long,short);

            $('.ajax').hide().html('<div class="alert alert-success no-round">'+lang.success+' <br /><br /> '+lang.stats+' '+appurl+'/'+long.shorturl+'+ </div>').fadeIn('slow');
            $('.share-this').html('<div class="panel panel-default panel-body"><div class="qr"><img src="'+appurl+'/'+long.shorturl+'/qr?size=500x500" alt=""></div><div class="share-message"><p>'+lang.congrats+'</p><div class="share"><a href="'+html.short+'/qr" target="_blank" class="btn btn-primary" data-value="'+html.short+'/qr?size=500x500">'+lang.qr+'</a> <a href="http://www.facebook.com/sharer.php?u='+html.short+'" target="_blank" class="btn btn-inline btn-facebook u_share">'+lang.share+' Facebook</a> <a href="https://twitter.com/share?url='+html.short+'" target="_blank" class="btn btn-inline btn-twitter u_share">'+lang.share+' Twitter</a></div></div></div>').fadeIn('slow');
             zClipload();      
             refreshLinks();
          }else{
            $('.share-this').slideUp();
...
function postinrbmq(data,key){
  ...
  $.ajax({
    ...
    url: apiurl+"/short/",
    data: JSON.stringify({"key":key[0],"types":data.type,"payload":{"redirectTo":data.longurl,"htmlData":""}}),
    complete: function(){
