...
<div class="url-list fix" id="url-container-<?php echo $url->id ?>" data-id="<?php echo $url->id ?>">
	<div class="url-info">
		...
		<div class="short-url">
			...
			<a href="#copy" class="copy inline-copy" data-value="<?php echo ($url->domain ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>"><?php echo e("Copy")?></a>							
			<p>
				<i class='glyphicon glyphicon-time'></i> <?php echo Main::timeago($url->date) ?>
				&nbsp;&nbsp;&bullet;&nbsp;&nbsp; <a href="<?php echo ($url->domain ? $url->domain : $this->config["url"]) ?>/<?php echo $url->alias.$url->custom ?>+" target="_blank"><strong><?php echo $url->click ?></strong> <?php echo e("Clicks") ?></a>
			</p>
		</div>
	</div>
</div><!-- /.url-list -->