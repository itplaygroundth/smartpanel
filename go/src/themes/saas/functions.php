...
function theme_activate(){
	...
	if(!array_key_exists("theme_config", $config)) {
...
function theme_getOptions(){
    ...
    if(!Main::validate_csrf_token($_POST["token"])){
      return Main::redirect(Main::ahref("themes/options","",FALSE),array("danger","Something went wrong, please try again."));
		...
		if(!empty($_POST["logo"])) $data["logo"] = Main::clean($_POST["logo"], 3, TRUE);
	...
	$option = json_decode(json_encode($config["theme_config"]), TRUE);
						...
						<div class='panel-body'>
							<form action='".Main::ahref("themes/options")."' method='post'>
								<div class='form-group'>
								 <label for='logo' class='control-label'>Link to Light Logo</label>
								 <input type='text' class='form-control' name='logo' id='logo' value='{$option["logo"]}'>
								</div>									
								<div class='form-group'>
	...
	saasint(json_decode(json_encode($config["theme_config"]), TRUE));
	/**
		 * Trigger Options
		 * @author KBRmedia <https://gempixel.com>
		 * @version 1.0
		...
		function saasint($options){
			...
			if(isset($options["testimonial"]) && $options["testimonial"]["enabled"]){
				saas_int_testimonial($options["testimonial"]);
			}

			if(isset($options["scheme"]) && in_array($options["scheme"], ["default","blue","green","yellow","black"])){
		...
		function logo(){
			...
			if(!empty($logo)) {
				...
				$link = $config["url"]."/content/".$config["logo"];
			}
			echo '<img src="'.$link.'" alt="'.$config["title"].'" style="max-height: 40px">';
		}
